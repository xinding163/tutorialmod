...
public class prospector extends Item
{
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            boolean found_ore=false;
            if (!Screen.hasShiftDown()) {//实现模糊搜索
                player.sendMessage(Text.of("imprecision:"));
                    ...
                    for (int j=-2;j<=2;j++){
                        for (int k=-2;k<=2;k++){
                            ...
                            if (isRightBlock(blockState)){
                                found_ore=true;
                                player.sendMessage(Text.of("Found " + name + " ores!"));
                            }
                        }
                    }
                }
                ...
                player.sendMessage(Text.of("Accuracy:"));
                for (int i=0;i<=pos.getY()+64;i++){
